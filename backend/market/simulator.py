"""The built-in market simulator — independent per-ticker geometric Brownian
motion (GBM). Used when MASSIVE_API_KEY is unset.

See planning/MARKET_SIMULATOR.md and planning/MARKET_DATA_DESIGN.md §10.

    S(t + dt) = S(t) * exp( (mu - 1/2 * sigma^2) * dt + sigma * sqrt(dt) * Z ),  Z ~ N(0, 1)

`dt` is expressed in trading years so an annualized volatility produces sane
per-tick moves; see MARKET_SIMULATOR.md §2 for the derivation.
"""
